...
public interface DocRequestRepo extends JpaRepository<DocRequest, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT date_trunc('day',doc_request.time_create), COUNT(*) AS totel FROM doc_request GROUP BY date_trunc('day',doc_request.time_create);")
